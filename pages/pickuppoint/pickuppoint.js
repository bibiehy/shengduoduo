...
import { fetchPickupList, fetchPickupDelete, fetchPickupDisabled } from '../../service/pickuppoint';
...
Page({
	...
	async onAjaxList(thisPage, callback) { // 列表请求
		...
		const result = await useRequest(() => fetchPickupList({ page: thisPage, name: keyword || '' }));
		if(result) {
			const newList = [];
	...
	async onDisabled(e) { // 禁用/启用
		...
		const result = await useRequest(() => fetchPickupDisabled({ id, type: status == 0 ? 1 : 0 }));
		if(result) {
	...
	async onDeleteSure() { // 确认删除
		...
		const result = await useRequest(() => fetchPickupDelete({ id: deleteItem['id'] }));
		if(result) {
