...
import { fetchProfitUser } from '../../../../service/user_storecenter';
...
Component({
	...
	methods: {
		...
		async onAjaxList(thisPage, callback) {
			...
			const result = await useRequest(() => fetchProfitUser(params));
			if(result) {
		...
		onShowDialog(e) {
			...
			const visible = type == 'fenjian' ? 'visibleFenjian' : 'visibleReport';
