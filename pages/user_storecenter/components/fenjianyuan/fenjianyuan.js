...
import { fetchFenList, fetchFenDelete, fetchFenDisabled } from '../../../../service/user_storecenter';
...
Component({
	...
	methods: {
		...
		async onDisabled(e) { // 禁用/启用
			...
			const result = await useRequest(() => fetchFenDisabled({ id, type: status == 1 ? 2 : 1 }));
			if(result) {
		...
		onCreate(e) { // 添加/编辑
			...
			wx.navigateTo({
				url: `/pages/user_storecenter/pages/fenjianyuan_create/fenjianyuan_create?type=${type}&strItem=${strItem}`,
				events: { // 注册事件监听器
		...
		async onDeleteSure() { // 确认删除
			...
			const result = await useRequest(() => fetchFenDelete({ id: deleteItem['id'] }));
			if(result) {
